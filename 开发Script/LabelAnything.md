...
b6aa407643326c0a7918c917e6c75940f1c4a910
conda activate label-anything

Loading dataset annotations from data/coco/annotations/sub_instances_train2014.json...
[2024-08-07 17:30:36] WARNING - coco.py - load_embeddings is not specified. Assuming load_embeddings=True.

data/coco/vit_b_sam_embeddings/last_block_state/000000038891.safetensors

```bash
python -m streamlit run app.py --server.port 6006

# 差了embeding
python main.py experiment --parameters="parameters/COCO.yaml"

# 训练特定文件夹的embeding
python main.py generate_embeddings --encoder vit_b --checkpoint checkpoints/sam_vit_b_01ec64.pth --use_sam_checkpoint --directory data/coco/train2017_subset --batch_size 4 --num_workers=8 --outfolder data/coco/vit_sam_embeddings/last_hidden_state --last_block_dir data/coco/vit_sam_embeddings/last_block_state --custom_preprocess


python main.py test --parameters="./test_parameters.yaml"

python main.py experiment --parameters="parameters_validation/COCO_Fold1_samvit.yaml"

python main.py experiment --parameters="test_validation.yaml"
...
python main.py generate_embeddings --encoder vit_b --checkpoint checkpoints/sam_vit_b_01ec64.pth --use_sam_checkpoint --directory data/coco/train_val_2017 --batch_size 4 --num_workers=8 --outfolder data/coco/vit_sam_embeddings/last_hidden_state --last_block_dir data/coco/vit_sam_embeddings/last_block_state --custom_preprocess
...
source activate label_anything
...
# 准备检查点
mkdir -p ../checkpoints/offline
cd ../checkpoints || exit
wget https://dl.fbaipublicfiles.com/segment_anything/sam_vit_b_01ec64.pth

# 可选：生成嵌入
mkdir -p data/coco/vit_sam_embeddings/last_hidden_state
mkdir -p data/coco/vit_sam_embeddings/last_block_state
...
[![PWC](%E5%BC%80%E5%8F%91Script/attachments/91518bb812119fdbd39bf55676150839_MD5.svg)](https://paperswithcode.com/sota/few-shot-semantic-segmentation-on-coco-20i-2-1?p=label-anything-multi-class-few-shot-semantic)
...
python main.py generate_embeddings --encoder vit_b --checkpoint checkpoints/sam_vit_b_01ec64.pth --use_sam_checkpoint --directory data/coco/train_val_2017 --batch_size 8 --num_workers=8 --outfolder data/coco/vit_sam_embeddings/last_hidden_state --last_block_dir data/coco/vit_sam_embeddings/last_block_state --custom_preprocess
```
